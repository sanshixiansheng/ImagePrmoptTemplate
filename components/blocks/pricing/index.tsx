...
export default function Pricing({ pricing }: { pricing: PricingType }) {
  ...
  return (
    ...
    >
      <div className="mx-auto mb-8 max-w-3xl px-4 text-center md:mb-10 md:px-8">
      ...
      <div className="container">
        <div className="relative z-10 mx-auto mt-6 mb-10 flex w-full justify-center md:mt-8 md:mb-12 md:max-w-lg">
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="inline-flex h-10 items-center justify-center rounded-full border border-border/60 bg-card/80 p-1 text-muted-foreground shadow-sm">
            ...
            return (
              <Card
                ...
                className={`text-card-foreground flex flex-col gap-6 rounded-xl py-6 relative overflow-visible border transition-all duration-300 backdrop-blur ${
                  isFeatured
                    ? "border-amber-200/70 bg-card shadow-[0_20px_60px_-25px_rgba(217,119,6,0.45)] ring-1 ring-amber-100/70 md:-translate-y-1 md:scale-[1.03] lg:scale-[1.06]"
              ...
              >
                ...
                <div
                  ...
                  className={`@container/card-header grid auto-rows-min grid-rows-[auto_auto] items-start gap-1.5 px-6 has-data-[slot=card-action]:grid-cols-[1fr_auto] [.border-b]:pb-6 pb-3 md:pb-4 ${
                    isFeatured ? "relative pt-5 md:pt-6" : ""
                  }`}
                >
                  <div data-slot="card-title" className="leading-none font-medium">
                    <h3 className={isFeatured ? "text-[17px] font-semibold md:text-lg" : "text-sm font-medium"}>
                      {item.title}
                    </h3>
                  </div>
                  ...
                  <div className="my-3 flex items-baseline gap-2">
                    ...
                    <div className={isFeatured ? "my-3 block font-semibold text-[30px] md:text-[34px]" : "my-3 block font-semibold text-2xl"}>
                      <span className={isFeatured ? "bg-gradient-to-r from-amber-500 via-yellow-500 to-amber-400 bg-clip-text text-transparent" : "text-primary"}>
                  ...
                  {isFeatured && item.badge && (
                    <div className="mt-2 flex flex-wrap items-center gap-2 text-xs font-medium text-amber-800">
                  ...
                  {isFeatured && item.limited_time_bonus && (
                    <div className="mt-3 flex items-center justify-between rounded-lg border border-amber-200/60 bg-amber-50/70 px-3 py-2 text-[12px] text-amber-900 shadow-sm">
